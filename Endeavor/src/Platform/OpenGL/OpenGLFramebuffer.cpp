...
namespace Endeavor {
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			ED_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Framebuffer only supports 4 attachments!");
		...
		ED_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			ED_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
